...
_ys = np.repeat(np.arange(L), L)    # y-coord of site i
...
_nn = np.array([[idx(_xs[i]+dx, _ys[i]+dy)
                 ...
                 for i in range(N)]) #Right Left Up Bottom
...
def compute_dos(E, eta=0.08, n_pts=1000):
    ...
    u_sq = (Uvec[:N, :]**2).sum(axis=0)  # Shape: (2N,)
...
for U in U_VALUES:
    ...
    for W in DISORDER_LIST:
        ...
        for r in range(N_DIS):
            ...
            dos, w      = compute_dos(E)
            dos_list.append(dos)
            delta_pool.extend(np.abs(Delta).tolist())   # |Delta_i|
...
plt.savefig("clean_distribution.png", bbox_inches='tight', dpi=500)
...
# fig
plt.savefig("clean_N(w).png", bbox_inches='tight', dpi=500)
...
for ax, W_map in zip(axes.flatten(), [0.0]):
    dmap = all_results[(U, W_map)]['Delta_map']
    vmax = all_results[(U, 0.0)]['delta_pool'].max() * 1.1

    # 'gray_r', 'plasma', 'viridis', 'magma', or 'inferno'
...
plt.savefig("phi_real_space_map_clean.png", bbox_inches='tight', dpi=500)
